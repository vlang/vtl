...
pub fn (mut layer BatchNorm1DLayer[T]) forward(input &autograd.Variable[T]) !&autograd.Variable[T] {
	if !input.requires_grad {
		// Inference path: use running stats
		output := internal.batchnorm1d_forward[T](input.value, layer.gamma.value, layer.beta.value, layer.running_mean, layer.running_var, layer.eps)!
		return input.context.variable(output)
	}
	// Training path: compute batch stats
	output, batch_mean, batch_var := internal.batchnorm1d_training[T](input.value, layer.gamma.value, layer.beta.value, layer.eps)!
	// Update running stats
	layer.running_mean = layer.running_mean.map(fn [layer] [T](val T, i []int) T {
		return vtl.cast[T](layer.momentum) * batch_mean + vtl.cast[T](1 - layer.momentum) * val
	})
	layer.running_var = layer.running_var.map(fn [layer] [T](val T, i []int) T {
		return vtl.cast[T](layer.momentum) * batch_var + vtl.cast[T](1 - layer.momentum) * val
	})
	mut result := input.context.variable(output)
	gate := batchnorm1d_gate[T](input.value, layer.gamma.value, layer.beta.value, batch_mean, batch_var, layer.eps)
...
pub fn batchnorm1d_gate[T](input &vtl.Tensor[T], gamma &vtl.Tensor[T], beta &vtl.Tensor[T], mean &vtl.Tensor[T], var_ &vtl.Tensor[T], eps f64) &BatchNorm1DGate[T] {
	return &BatchNorm1DGate[T]{
		input: input, gamma: gamma, beta: beta, mean: mean, var: var_, eps: eps
	}
}

pub fn (g &BatchNorm1DGate[T]) backward[T](payload &autograd.Payload[T]) ![]&vtl.Tensor[T] {
	return internal.batchnorm1d_backward[T](payload.variable.grad, g.input, g.gamma, g.beta, g.mean, g.var, g.eps)
}
...
pub fn (g &BatchNorm1DGate[T]) cache[T](mut result autograd.Variable[T], args ...autograd.CacheParam) ! {
	match args[0] {
		autograd.Variable[T] {
			...
			autograd.register[T]('BatchNorm1D', g, result, [args[0]])!
		}
		else {}
	}
}